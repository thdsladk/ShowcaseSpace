...
#include "TargetActor/Skill/TA_Skill_Radius.h"
...
FGameplayAbilityTargetDataHandle ATA_Skill_Radius::MakeTargetData() const
{
	...
	if (UJWFunctionLibrary::IsDebugMode(SourceActor) == true)
	{
		...
		Shape.MakeSphere(100.f);
		Shape.SetShape(ECollisionShape::Sphere, FVector{ 100.f,0.f,0.f });
		for (const FOverlapResult& Overlap : Overlaps)
		{
			UJWFunctionLibrary::Debug_ShapeSingleByChannel(SourceActor, Overlap.GetActor()->GetActorLocation(), Shape, HitDetected);
		}
	}
